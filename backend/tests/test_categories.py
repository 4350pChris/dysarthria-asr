...
def test_category_endpoint_renames_and_deletes_its_phrases(initialized_db: Path) -> None:
    ...
    created_category = client.post("/api/categories", json={"name": "Test"})
    assert created_category.status_code == 201
    category = created_category.json()
    duplicate_category = client.post("/api/categories", json={"name": "Test"})
    ...
    created_phrase = client.post("/api/phrases", json={"category_id": category["id"], "text": "Hallo"})
    assert created_phrase.status_code == 201

    renamed = client.patch(f"/api/categories/{category['id']}", json={"name": "Neu"})
...
def test_phrase_routes_return_conflicts_and_no_content(initialized_db: Path) -> None:
    client = TestClient(create_app())
    category = client.post("/api/categories", json={"name": "Test"}).json()
    first = client.post("/api/phrases", json={"category_id": category["id"], "text": "Hallo"})
    second = client.post("/api/phrases", json={"category_id": category["id"], "text": "Tschüss"})

    assert first.status_code == 201
    assert second.status_code == 201
    duplicate = client.post(
        "/api/phrases",
        json={"category_id": category["id"], "text": "Hallo"},
    )
