...
async def create_category(name: str = Form(...)) -> dict:
...
async def create_phrase(category_id: int = Form(...), text: str = Form(...)) -> dict:
...
async def update_phrase(phrase_id: int, text: str = Form(...)) -> dict:
    clean_text = text.strip()
    if not clean_text:
        raise HTTPException(status_code=400, detail="Phrase text is required.")
    with connect_db() as db:
